...
- `fast_voice.py`: provides an isolated experimental low-latency path with Enter, direct, or local clap activation; honors `VOICE_INPUT_DEVICE`; stops command recording after configurable silence; performs one STT call; reuses speech repair, validation, `AssistantCore`, OpenAI routing, and optional TTS; reports per-stage timing; and writes `logs/fast_voice.log`. It does not alter the GUI, wake providers, or full voice loop.
