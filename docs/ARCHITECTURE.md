...
- `voice_loop.py`: runs the continuous voice loop, uses the selected wake provider, falls back to Whisper fuzzy wake detection when needed, rejects bad command transcripts locally with one retry by default, handles accepted commands through `AssistantCore`, speaks accepted assistant responses, enters one short follow-up listening window before sleeping, accepts a valid follow-up command without another wake phrase, emits rejected/retry/follow-up/accepted/response/sleep events for the GUI, handles stop commands, applies post-speech cooldowns, reports summary counters, and writes `logs/voice_loop.log`.
